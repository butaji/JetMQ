...
class WillSpec extends TestKit(ActorSystem()) with ImplicitSender with SpecificationLike with Scope {
  ...
  "Requests handler actor" should {
    ...
    "Scenario 53183" in {
      val h = create_actor("53183")

      h ! "103800044d51545404160002000a6d79636c69656e7469640007546f7069632f430017636c69656e74206e6f7420646973636f6e6e6563746564".toTcpReceived //Connect(Header(false,0,false),ConnectFlags(false,false,false,2,true,true,2),myclientid,Some(Topic/C),Some(client not disconnected),None,None)
      expectMsg("20020000".toTcpWrite) //Connack(Header(false,0,false),0)
      expectNoMsg(Bag.wait_time)
      success
    }
    ...
    "Scenario 53184" in {
      ...
      h ! "820c00020007546f7069632f4302".toTcpReceived //Subscribe(Header(false,1,false),2,Vector((Topic/C,2)))
